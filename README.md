...
| `npm run audit:wire` | Point a production build at a stub backend that answers *nearly* right, and check what the customer is told |
...
| `npm run audit:firstrun` | Walk the whole first run in order — splash, carousel, sign-in, guest, location permission, Home — both answers to the permission, and check the app does not ask a second time |
...
| `npm run audit:wide` | Render every busy screen at both iPad shapes and after a mid-session resize, and fail on overflow or a hero taller than the viewport |
...
while any of the 28 catalogue products lack their own artwork, so it can gate a
release build.

### Current asset status

**All 28 of 28** catalogue products carry their own supplied bb.q photograph.
Nothing borrows, nothing renders a placeholder, and `npm run assets:audit`
exits clean:

```
All 28 catalogue products have supplied artwork. Cleared for production.
```
...
`npm run audit:screens` on all 69 routes, rather than asserted here.
